...
if __name__ == '__main__':
    ...
    for i, composition in enumerate(['gdp', 'ma']):
        ...
        for iteration in tqdm(iterations):
            ...
            if i == 0:
                """
                if epsilon >= threshold and not plotted_examples:
                    temp_key, key = random.split(key)
                    X_syn = sample(temp_key, params, X_test.shape[0])
                    for dim in range(X_syn.shape[1]):
                        fig = plt.figure(figsize=(4, 3))
                        ax = fig.add_subplot()
                        ax.hist(X_test[:, dim], color='blue', alpha=0.6, bins=num_bins, range=(-0.5, 0.5), label='Real', rasterized=True)
                        ax.hist(X_syn[:, dim], color='orange', alpha=0.6, bins=num_bins, range=(-0.5, 0.5), label='Synthetic', rasterized=True)
                        plt.legend()
                        plt.grid(True)
                        plt.tight_layout()
                        plt.savefig('flow_dimwise_plots/' + str(dim) + '.png', dpi=dpi)
                        plt.close('all')
                    plotted_examples = True
                """
    ...
    for name, dirname in [('DP-MoG (MA)', 'DP-EM-MA'), ('DP-MoG (zCDP)', 'DP-EM-zCDP')]:
        ...
        for filename in os.listdir(matfiledir):
            ...
            if dirname == 'DP-EM-MA' and epsilon == threshold:
                """
                print('Plotted MoG examples!')
                ...
                for dim in range(X_syn.shape[1]):
                    ...
                    plt.grid(True)
                    plt.tight_layout()
                    plt.savefig('mog_dimwise_plots/' + str(dim) + '.png', dpi=dpi)
                    plt.close('all')
                """
